const CACHE_NAME="hstu-resource-centre-v20260925-gallery-single";
const CORE=[
...
/* Keep ONLY the Gallery CTA inside the moving collage. Hide the older duplicate below the hero. */
.hst-gallery-cta-wrap{display:none!important}
...
(function(){
 function install(){
   var old=document.querySelector('.hst-gallery-cta-wrap'); if(old) old.style.display='none';
   var host=document.querySelector('.hstu-collage-brand'); if(!host) return;
   ...
   var b=document.getElementById('liveGoGallery'); if(!b) return;
   b.addEventListener('click',function(){
     var nav=document.querySelector('[data-view="gallery"]'); if(nav){nav.click();return;}
...
self.addEventListener("fetch",event=>{
 ...
 if(event.request.mode==="navigate"){
  event.respondWith(fetch(event.request,{cache:"no-store"}).then(async response=>{
   ...
   caches.open(CACHE_NAME).then(cache=>cache.put("./index.html",out.clone())); return out;
 ...
 if(fresh){event.respondWith(fetch(event.request,{cache:"reload"}).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put(event.request,copy));return response}).catch(()=>caches.match(event.request)));return}
